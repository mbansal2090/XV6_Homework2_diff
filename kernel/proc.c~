...
int
getpinfo(struct pstat* structlist){
  struct proc *p;
  ...
  for(i = 0, p = ptable.proc; p < &ptable.proc[NPROC]; p++, i++){
	structlist->pid[i] = p->pid;
    structlist->lticks[i] = p->ltick;
	structlist->hticks[i] = p->htick;
    if(p->state == UNUSED){
	  structlist->inuse[i] = 0;
    } else {
	  structlist->inuse[i] = 1;
    }
  }  
  release(&ptable.lock);
  return 0;
...
found:
  ...
  p->ltick = 0; // default low tick = 0 (because process has not run till now)
  p->htick = 0; // default high tick = 0 (because process has not run till now)
  release(&ptable.lock);
